...
ZeroTier Inc runs controllers at with a Web UI and API at [ZeroTier Central](https://my.zerotier.com).

You can [self-host controllers](./network-controllers), but we don't currently offer the web UI for self-hosting. You'll have to use the json api. 
...
Introductions for peer to peer connections happen via the roots. If p2p connections can't be made and traffic is "relayed" through these roots.

Remember: All traffic is end-to-end encrypted. The roots can't read anything that passes through them. 

You can [host your own roots](../zerotier/moons) in addition to ours, but you can't not use ours' (unless your are a Very Large Enterprise Customer) and the mobile apps don't support custom roots. 
