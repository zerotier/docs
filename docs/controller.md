...
:::info OpenAPI
The setup described here uses the local [ZeroTierOne service API](/api-service) to provision and manage networks. You can [browse](/api/service/v1/) the OpenAPI docs for the local controller API for more detail on this interface.
:::
...
You'll need to find the current local auth token on your system using the instructions in the [API Tokens](/tokens#zerotierone-service-token) guide.
